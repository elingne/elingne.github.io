...
window.spaNavigate = function(path, replace=false) {
  ...
  else if (location.hash === next) renderSpaRoute();
...
async function renderSpaRoute() {
  ...
  try {
    if (route.view === "main") await runViewScript("app-v8.js", token);
    if (route.view === "character") await runViewScript("character-v6.js", token);
    if (route.view === "pair") await runViewScript("pair-v6.js", token);
...
window.addEventListener("hashchange", renderSpaRoute);
window.addEventListener("popstate", () => { if (location.hash.startsWith("#/")) renderSpaRoute(); });
